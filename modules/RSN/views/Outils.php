...
class RSN_Outils_View extends Vtiger_Index_View {
	...
	function updateGroupeDescriptifFromDonnes4D(){
		...
		$query = 'SELECT crmid, vtiger_modcomments.commentcontent, vtiger_contactscf.grpnomcourt, vtiger_contactaddress.mailingstreet2
			...
			AND vtiger_modcomments.commentcontent LIKE "Donn%associationcourt%"
			ORDER BY crmid
			LIMIT '. $limit;
		...
		do {
			...
			while($row = $adb->getNextRow($result, false)){
				...
				if($associationCourt && !preg_match('/^Donn.*es 4D/', $associationCourt)){
					...
					if( $associationCourt == $row['mailingstreet2'] && $associationCourt != $row['grpnomcourt'] ){
						var_dump($associationCourt . ' / ' . $row['grpnomcourt'] );
					}
					if( $associationCourt && $associationCourt != $row['grpnomcourt'] ){
						$updCounter++;
						$resultUpd = $adb->pquery($queryUpdate, array($associationCourt, $row['crmid']));
						if(!$resultUpd){
