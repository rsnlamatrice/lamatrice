...
class Vtiger_Module_Model extends Vtiger_Module {
	...
	public function getFields() {
		if(empty($this->fields)){
			...
			$this->fields = array();
			/* la double boucle perturbe l'ordre des champs */
	...
	public static function getAll($presence = array(), $restrictedModulesList = array(), $roleid = NULL) {
		...
		if(!$moduleModels){
			...
			/* ED141226
			 *	recherche de tabsequence dans la nouvelle table vtiger_rsnroletabsequence
			 */
			...
			';
			if($roleid){
				$query .= ' LEFT JOIN vtiger_rsnroletabsequence
				ON vtiger_rsnroletabsequence.`tabid` = vtiger_tab.`tabid`
				AND vtiger_rsnroletabsequence.roleid = ?';
...
//			);
	...
	public function getRelationQuery($recordId, $functionName, $relatedModule) {
		...
		//var_dump('$this->getName()', $this->getName());
		$focus = CRMEntity::getInstance($this->getName());
		$focus->id = $recordId;
	...
	*/ 
	...
	public function getPicklistValuesDetails($fieldname){
		...
		return $recordModel->getPicklistValuesDetails($fieldname); // la définition s'effectue, historiquement parlant, dans le Record_Model
	}
