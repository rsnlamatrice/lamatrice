...
class RSNImport_Import_View extends Vtiger_View_Controller{
	 ...
	 *  By default it return only the curent module.
	...
	public function getPreviewData() {
		...
		foreach($importModules as $module) {
			...
			$sql .= ' FROM ' . $tableName . ' WHERE status = '. RSNImport_Data_Action::$IMPORT_RECORD_NONE . ' LIMIT 12';
	...
	function displayDataPreview() {
		...
		return $viewer->view('ImportPreview.tpl', 'RSNImport');
	}
	...
	function clearPreImportTable() {
		...
		foreach ($modules as $module) {
			RSNImport_Utils_Helper::clearUserImportInfo($this->user, $module);
			RSNImport_Utils_Helper::createTable($this->getFieldsFor($module), $this->user, $module);
		}
	}
	...
	public function queueDataImport($module) {
		RSNImport_Queue_Action::add($this->request, $this->user, $module, $this->getMappingFor($module));
	}
	...
	public function undoImport($userId) {
        ...
        $viewer->view('okButton.tpl', 'RSNImport');
	...
	function doUndoImport($module, $user) {
		...
		if (method_exists($this, $methode)) {
			...
			$tableName = RSNImport_Utils_Helper::getDbTableName($user, $module);
			$query = "SELECT recordid FROM " . $tableName . " WHERE status = " . RSNImport_Data_Action::$IMPORT_RECORD_CREATED . " AND recordid IS NOT NULL";
			...
			$viewer->view('ImportUndoResult.tpl', 'RSNImport');
		}
	}
	...
	public static function showImportStatus($importInfos, $user, $moduleName = "") {
		...
		if ($importEnded) {
			$viewer->view('EndedImportButtons.tpl', 'RSNImport');
		} else {
			$viewer->assign('IMPORT_SOURCE', $importInfos[0]['importsourceclass']);
