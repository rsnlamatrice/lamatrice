...
class Accounts extends CRMEntity {
	...
	*/
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=> 'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
		/*$contactDetailsSql = "vtiger_contactdetails.`contactid`, vtiger_contactdetails.`contact_no`, vtiger_contactdetails.`accountid`
		, vtiger_contactdetails.`salutation`, vtiger_contactdetails.`firstname`, vtiger_contactdetails.`lastname`, vtiger_contactdetails.`email`, vtiger_contactdetails.`phone`, vtiger_contactdetails.`mobile`, vtiger_contactdetails.`title`
		, vtiger_contactdetails.`department`, vtiger_contactdetails.`fax`, vtiger_contactdetails.`reportsto`, vtiger_contactdetails.`training`, vtiger_contactdetails.`usertype`
		, vtiger_contactdetails.`isgroup`, vtiger_contactdetails.`contacttype`, vtiger_contactdetails.`otheremail`, vtiger_contactdetails.`secondaryemail`, vtiger_contactdetails.`donotcall`, vtiger_contactdetails.`emailoptout`, vtiger_contactdetails.`donotprospect`, vtiger_contactdetails.`imagename`
		, IF(vtiger_contactdetails.`accountid` = '".$id."', `reference`, '0') AS `reference`
		, vtiger_contactdetails.`notify_owner`, vtiger_contactdetails.`isconvertedfromlead`";*/
	...
	*/
	function get_invoices($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
				vtiger_crmentity.*
				, vtiger_invoice.*
				, vtiger_account.accountname, vtiger_salesorder.subject AS salessubject
				FROM vtiger_invoice
	...
	*/
	function get_criteres4D($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
					...
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
					where vtiger_contactdetails.accountid=".$id." and vtiger_crmentity.deleted=0";
		...
		$query = "
			SELECT CASE WHEN (vtiger_users.user_name not like '') THEN $userNameSql ELSE vtiger_groups.groupname END   AS user_name,
					...
					FROM (
						...
						WHERE vtiger_contactdetails.accountid=".$id."
