...
class Invoice_Record_Model extends Inventory_Record_Model {
	...
	/**
	 * ED151022
	 * Mise  jour des lments de rglements d'aprs les RsnReglements lis
	 */
	public function updateReceivedFromRelated(){
		if(str_to_float($this->get('received')))
			return 'Cette facture contient dj un montant de rglement. Veuillez modifier manuellement le montant.'; //dj une affectation
		...
		//Recherche des rglements relatifs qui ne soient pas lis  d'autres factures
		$query = 'SELECT vtiger_rsnreglements.rsnreglementsid, vtiger_rsnreglements.numpiece, vtiger_rsnreglements.rsnmoderegl, vtiger_rsnreglements.dateregl
			...
			LEFT JOIN ( /* Calcul des utilisations des mmes rglements pour d autres factures */
		...
		//$this->save(); ne fonctionne pas, il faut initialiser toutes les donnes des lignes
		...
		switch($fieldname){
			...
			case 'invoicestatus':
				return array(
					...
					'Compta' => array( 'label' => 'Archivé', 'icon' => 'ui-icon ui-icon-arrowthick-1-e blue' ),
