...
class Services extends CRMEntity {
	...
	var $sortby_fields = Array('sortindex', 'servicename');
	...
	*/
	 ...
	 * Get list view query.
	 */
	function getListQuery($module, $where='', $with_taxes = FALSE) {
		...
		/* ED141211 taxe
		 * TODO : attention, si plusieurs taxe
 		 */
		if($with_taxes)
			$query .= ", vtiger_producttaxrel.taxid, vtiger_producttaxrel.taxpercentage";
		...
		/* ED141211 taxe */
		if($with_taxes)
			$query .= " LEFT JOIN vtiger_producttaxrel
				ON vtiger_producttaxrel.productid = vtiger_crmentity.crmid ";
	...
	function get_invoices($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_crmentity.*,
			...
			LEFT OUTER JOIN vtiger_invoicecf
