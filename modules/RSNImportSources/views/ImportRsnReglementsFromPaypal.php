...
class RSNImportSources_ImportRsnReglementsFromPaypal_View extends RSNImportSources_ImportFromFile_View
{
    ...
    {
        if (
            $this->request->get('mode') === 'validatePreImportData'
    ...
    function importOneContacts($contactsData, $importDataController)
    {
        ...
        if (is_numeric($entryId)) {
            ...
            $result = $db->pquery($query, array(
                ASSIGNEDTO_ALL, $contactsData[0]['date'], $contactId
            ));
        ...
        if ($invoice != null) {
            ...
            if ($accountId != null) {
                ...
                if ($db->num_rows($result)) {
                    ...
                    $record->set('clicksource', $reglement['clicksource']);
        ...
        if ($contact != null) {
            ...
            if ($account != null) {
                ...
                if ($db->num_rows($result)) {
                    ...
                    $result = $db->pquery($query, array(
                        $invoiceNo, $total, $total, 'individual', ASSIGNEDTO_ALL
                        //, $invoiceData[0]['invoicedate']
                        //, $invoiceData[0]['invoicedate']
                        , $invoiceId
                    ));
    ...
    private function updateContactRecordModelFromData($record, $contactsData)
    {
        ...
        if (
            !$record->get('mailingzip') || !$record->get('mailingcity')
    ...
    function postPreImportInvoiceData()
    {
        ...
        , `_invoiceid` = vtiger_crmentity.crmid
        ,  `_contactid` = vtiger_invoice.contactid
        WHERE vtiger_crmentity.deleted = 0
        AND vtiger_invoice.contactid IS NOT NULL AND vtiger_invoice.contactid <> 0
        AND `$tableName`.status = " . RSNImportSources_Data_Action::$IMPORT_RECORD_NONE . "
