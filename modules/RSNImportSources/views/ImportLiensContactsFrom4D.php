...
';
		...
		if ($contactId && $relContactId) {
			...
			$query = "SELECT 1
				...
				JOIN vtiger_crmentity vtiger_crmentity_rel
					ON vtiger_crmentity_rel.relcontid = vtiger_crmentity_rel.crmid
				WHERE ((contactid = ?
					AND relcontid = ?)
				OR (relcontid = ?
					AND contactid = ?)
	...
	private function updateContactsRecordModelFromData($record, $relatedcontactsData){
		
		$fieldsMapping = $this->getContactsFieldsMapping();
		foreach($relatedcontactsData[0] as $fieldName => $value)
