...
{strip}
	...
	<td>
		...
		<div>
			<input type="text" id="{$productName}" name="{$productName}" value="{if $data.$hdnProductId}{$data.$productCode} - {$data.$productName}{/if}" class="productName {if $row_no neq 0} autoComplete {/if}" placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}" data-validation-engine="validate[required]" {if !empty($data.$productName)} disabled="disabled" {/if}/>
