...
{strip}
	...
	<td>
		...
		<div>
			<input type="text" id="{$productName}" name="{$productName}" value="{if $data.$hdnProductId}{$data.$productCode} - {$data.$productName}{/if}"
				    title="{if $data.$hdnProductId}{$data.$productCode} - {$data.$productName}{/if}"
					class="productName {if $row_no neq 0} autoComplete {/if}" placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}" data-validation-engine="validate[required]"
	...
	<td>
		<div class="edit-Price">
			<input id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$listPrice)}{(float)$data.$listPrice}{else}0{/if}" type="text" data-validation-engine="validate[required,funcCall[Vtiger_PositiveNumber_Validator_Js.invokeValidation]]"
