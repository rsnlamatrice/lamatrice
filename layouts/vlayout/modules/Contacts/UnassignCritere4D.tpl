...
<div id="massEditContainer" class='modelContainer'>
	<div class="modal-header contentsBackground">
		...
		<h3 id="massEditHeader">{if count($RELATED_ENTRIES) > 1}{vtranslate('LBL_UNASSIGN_CRITERE4D_MULTI', $MODULE)}{else}{vtranslate('LBL_UNASSIGN_CRITERE4D', $MODULE)}{/if}
			&nbsp;{vtranslate('LBL_IN_RELATION_WITH', $MODULE)} {$UNASSIGNABLE_COUNTER} {vtranslate($MODULE, $MODULE)}</h3>
	</div>
	<form class="form-horizontal" id="massEdit" name="MassEdit" method="post" action="index.php">
		<input type="hidden" name="module" value="{$MODULE}" />
		<input type="hidden" name="action" value="UnassignCritere4D" />
