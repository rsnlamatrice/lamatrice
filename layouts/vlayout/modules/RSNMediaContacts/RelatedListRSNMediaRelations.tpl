    ...
    <div class="bottomscroll-div">
        ...
        <table class="table table-bordered listViewEntriesTable">
			...
			<th {if $HEADER_FIELD@last} colspan="2" {/if} nowrap class="{$WIDTHTYPE}">
			    ...
			    {if $IS_BUTTONSET}
				    {assign var=tmp value=$HEADER_FIELD->set('picklist_values',$RELATED_MODULE->getListViewPicklistValues($RELATED_HEADERNAME))}
			    {/if}
		...
		{assign var=UICOLOR value=$RELATED_RECORD->get('uicolor')}
			...
			{assign var=UITYPE value=$HEADER_FIELD->get('uitype')}
			{assign var=IS_BUTTONSET value=$UITYPE eq '402'}
			    ...
			    {/if}>
					...
					&nbsp;{$PICKLIST_LABEL}
				    {/if}</label>
			    {elseif $HEADER_FIELD->isNameField() eq true or $UITYPE eq '4'}
