if (typeof(RSNImportJs) == 'undefined') {
    /*
    ...
    RSNImportJs = {
    	...
    	registerEvent: function() {
    		RSNImportJs.registerSelectSourceEvent();
    		RSNImportJs.registerBasicNextButtonEvent();
    	},

    	registerSelectSourceEvent: function() {
			jQuery('#SelectSourceDropdown').on('change', function(e){
				$('button[name="next"]').removeAttr('disabled');
				RSNImportJs.loadSourceConfiguration(this.value);
			});
		},
		...
		registerBasicNextButtonEvent: function() {
			...
			if (typeof sourceDropdown !== "undefined") { // tmp valide only for first view !!
				if (!sourceDropdown.val()) {
					...
					RSNImportJs.loadSourceConfiguration(jQuery('#SelectSourceDropdown').val());
				}
			}
			$('button[name="next"]').on('click', function(e) {
				RSNImportJs.preImport(e);
			});
		},

		registerFileConfigurationEvent: function() {
			RSNImportJs.registerAdvancedFileConfigurationEvent();
			RSNImportJs.registerFileConfigurationChangeEvent();
		},
		...
		registerFileConfigurationChangeEvent: function() {
			jQuery("#import_file").on('change', function(e) {
				RSNImportJs.checkFileType(e);
			});


			RSNImportJs.handleNeededFileTypeChange();
			jQuery("#file_type").on('change', function(e) {
				RSNImportJs.handleNeededFileTypeChange(e);
			});

			jQuery("#file_encoding").on('change', function(e) {
				RSNImportJs.handleNeededFileEncodingChange(e);
			});

			jQuery("#file_delimiter").on('change', function(e) {
				RSNImportJs.handleNeededFileDelimiterChange(e);
			});
		},
		...
		loadSourceConfiguration: function(view) {
			var data = {
				...
				for_module: RSNImportJs.getForModuleName(),
		        view: view,
		    ...
		    function(data, status) {
		    	if (status == "success") {
					...
					if (typeof RSNImportJs[onLoadFunction] === "function") {
						return RSNImportJs[onLoadFunction]();
					}
		    	}
		    });
		},
		...
		preImport: function(e) {
			...
			if (typeof RSNImportJs[validateFunctionName] === "function") {
				return RSNImportJs[validateFunctionName](e);
			}
		...
		validateFile: function(e) {
			if (!RSNImportJs.validateFilePath()) {
				e.preventDefault();
				return false;
			}
		...
		validateFilePath: function() { // tmp !!!!
			...
			if(!RSNImportJs.validateFileType("import_file", "curent_file_type", "file_type")) {
				return false;
			}
			if(!RSNImportJs.validateUploadFileSize("import_file")) {
				return false;
			}
			return true;
		},
		...
		checkFileType: function() {
			...
			if(filePath != '') {
				...
				//RSNImportJs.handleFileTypeChange();
			}
		},
