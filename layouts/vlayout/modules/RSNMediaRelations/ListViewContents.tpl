...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable">
		<thead>
			<tr class="listViewHeaders">
				...
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					...
					{if $IS_BUTTONSET}
						{assign var=tmp value=$LISTVIEW_HEADER->set('picklist_values',$RECORD_MODEL->getListViewPicklistValues($LISTVIEW_HEADER->getFieldName()))}
					{/if}
		...
		{assign var=DATERELATION value=$LISTVIEW_ENTRY->get('daterelation')}
		<tr class="listViewEntries {if $PREVIOUS_DATE && ($PREVIOUS_DATE neq $DATERELATION)}new-day{/if}"
		...
		>
			{assign var=PREVIOUS_DATE value=$DATERELATION}
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}"
			    ...
			    {if $IS_BUTTONSET} style="padding-left: 1em;"{/if}
			    nowrap>
				{* ED141010 *}
				{if $IS_BUTTONSET}
					...
					{else}
					    &nbsp;{$PICKLIST_LABEL}
					{/if}</label>
				{else if $LISTVIEW_HEADER->isNameField() eq true or $UITYPE eq '4'}
