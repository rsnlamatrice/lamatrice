		...
		<span class="row-fluid">
			<span class="recordLabel font-x-x-large textOverflowEllipsis span pushDown" title="{$RECORD->getName()}">
				{foreach item=NAME_FIELD from=$MODULE_MODEL->getNameFields()}{*$NAME_FIELD*}
					...
					{/if}
				{/foreach}
				{if $RECORD->get('sostatus') eq 'Cancelled'}
					<br><span style="color: red;">{vtranslate($RECORD->get('typedossier'), $MODULE )} - {vtranslate('Cancelled')}</span>
				{else}
					<br><span style="color: blue;">{vtranslate($RECORD->get('typedossier'), $MODULE )}</span>
				{/if}
			</span>
		</span>
	</span>
