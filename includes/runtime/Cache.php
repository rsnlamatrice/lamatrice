...
class Vtiger_Cache  {
    ...
    public static $sessionCacheEnable = false;
    ...
    */
    ...
    public function getPicklistValues($fieldName, &$picklistvaluesdata = FALSE){
	    if(isset(self::$_picklist_values[$fieldName])){
		    $picklistvaluesdata = $this->getPicklistData($fieldName);
		    return self::$_picklist_values[$fieldName];
	    }
	    return false;
    }

    public function setPicklistValues($fieldName,$values, &$picklistvaluesdata = FALSE){
	    if(self::$cacheEnable){
		    self::$_picklist_values[$fieldName]=$values;
		    if(is_array($picklistvaluesdata))
			$this->setPicklistData($fieldName, $picklistvaluesdata);
	    }
    }

    /* ED141128 */
    private static $_picklist_data;

    public function getPicklistData($fieldName){
	    if(isset(self::$_picklist_data[$fieldName])){
		    return self::$_picklist_data[$fieldName];
	    }
	    return false;
    }

    public function setPicklistData($fieldName,$values){
	    if(self::$cacheEnable){
		    self::$_picklist_data[$fieldName]=$values;
	    }
    }

    /* ED141128 */
    private static $_picklist_uicolumns;

    public function getPicklistUIColumns($fieldName){
	    if(isset(self::$_picklist_uicolumns[$fieldName])){
		    return self::$_picklist_uicolumns[$fieldName];
	    }
	    return false;
    }

    public function setPicklistUIColumns($fieldName,$values){
	    if(self::$cacheEnable){
		    self::$_picklist_uicolumns[$fieldName]=$values;
	    }
    }
    ...
    /* ED141128 */
    private static $_picklist_properties;

    public function getPicklistProperties($tableName,$roleId){
	    if(isset(self::$_picklist_properties[$tableName][$roleId])){
		    return self::$_picklist_properties[$tableName][$roleId];
	    }
	    return false;
    }

    public function setPicklistProperties($tableName,$roleId,$values){
	    if(self::$cacheEnable){
		    self::$_picklist_properties[$tableName][$roleId]=$values;
	    }
    }

    public function hasPicklistProperties($tableName, $roleId) {
	    $values = $this->getPicklistProperties($tableName, $roleId);
	    return $values !== false;
    }
